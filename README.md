...
The **OpenSubtitles Hash** (OSHash) is a fast file identification algorithm used by [OpenSubtitles](https://www.opensubtitles.org) to match video files with subtitles. This repository contains **55 verified implementations** across every major programming language, a test suite, and a reference website.
...
| **Functional** | [Haskell](implementations/haskell/oshash.hs), [OCaml](implementations/ocaml/oshash.ml), [Common Lisp](implementations/lisp/oshash.lisp), [Erlang](implementations/erlang/oshash.erl), [Scheme](implementations/scheme/oshash.scm), [Racket](implementations/racket/oshash.rkt), [Standard ML](implementations/sml/oshash.sml) |
| **Logic / OO / stack** | [Prolog](implementations/prolog/oshash.pl), [Smalltalk](implementations/smalltalk/oshash.st), [Forth](implementations/forth/oshash.fs), [Rexx](implementations/rexx/oshash.rexx) |
...
> **Ada**, **Algol 68**, **COBOL**, **Forth**, **Modula-2**, **Objective-C**, **Odin**, **Pony**, **Prolog**, **Racket**, **Rexx**, **Scheme** (Gambit), **Smalltalk** and **Standard ML** are built inside a throwaway Docker image (no host toolchain) — run `bash tools/build_docker_langs.sh` once; pass `--rmi` to delete the image afterwards. The resulting binaries run on the host on their own.
